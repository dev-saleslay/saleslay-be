...
/**
 * Only includes CRM scalars HubSpot actually sent on this payload, so we do not wipe
 * existing SalesLay values when a property is omitted from the API response.
 */
...
/** Safety cap: 500 pages × 100 limit = 50k contacts per full sync. */
const HUBSPOT_SYNC_MAX_PAGES = 500;

/**
 * Deletes local HubSpot leads whose contact id was not returned in the latest full HubSpot crawl.
 */
...
export async function syncHubSpotContacts({
  ...
  maxPages?: number;
  /**
   * When true: fetch every page from HubSpot (up to a safety cap), then delete local HUBSPOT leads
   * whose `externalId` never appeared in that crawl (removed from HubSpot or no longer visible).
   * Ignores `after` — always starts from the first page.
   */
  ...
  /** Only true when HubSpot reports no further pages — required before pruning local rows. */
